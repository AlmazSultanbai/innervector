...
export async function POST(req: NextRequest) {
  try {
    ...
    const langInstruction =
      lang === 'ru'
        ? 'IMPORTANT: Respond ENTIRELY in Russian. Every single text value in the JSON must be written in Russian. Do not use any English words except for proper names of people and the dominantDomain field. CRITICAL: Every time you mention a CliftonStrengths theme name in the text, write it as: EnglishName (Русский перевод) — for example: "Self-Assurance (Уверенность)", "Futuristic (Будущее)", "Individualization (Индивидуализация)", "Strategic (Стратегия)", "Ideation (Генератор идей)", "Learner (Ученик)", "Deliberative (Осторожность)", "Competition (Конкуренция)", "Focus (Сосредоточенность)", "Achiever (Достижение)", "Activator (Катализатор)", "Adaptability (Приспособляемость)", "Analytical (Аналитик)", "Arranger (Организатор)", "Belief (Убеждение)", "Command (Распорядитель)", "Communication (Коммуникация)", "Connectedness (Взаимосвязанность)", "Consistency (Последовательность)", "Context (Контекст)", "Developer (Развитие)", "Discipline (Дисциплинированность)", "Empathy (Эмпатия)", "Harmony (Гармония)", "Includer (Включенность)", "Input (Вклад)", "Intellection (Мышление)", "Maximizer (Максимизатор)", "Positivity (Позитивность)", "Relator (Отношения)", "Responsibility (Ответственность)", "Restorative (Восстановление)", "Self-Assurance (Уверенность)", "Significance (Значимость)", "Woo (Обаяние)". Always show both: the English name first, then the Russian translation in parentheses.'
        : lang === 'ky'
