...
export default function HistoryPage() {
  ...
  const generateAnalysis = async (r: VectorTestRecord) => {
    ...
    try {
      ...
      const res = await fetch('/api/vector-analyze', {
        ...
        body: JSON.stringify({ top5, top10, bottom5, lang: r.lang ?? 'ru', full_name: r.full_name ?? '', result_id: r.id }),
      });
      const data = await res.json();
      if (!data.error) {
        setVectorResults(prev => prev.map(v => v.id === r.id ? { ...v, analysis: data } : v));
      }
    } finally {
                        ...
                        {isSuperAdmin && (<>
                          {!r.analysis && (
                            <button
                              ...
                              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-violet-500/10 border border-violet-500/25 text-violet-400 text-xs font-medium hover:bg-violet-500/20 transition-all duration-200 disabled:opacity-50"
                            >
                              {generatingId === r.id ? (
