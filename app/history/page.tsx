...
async function downloadPDF(analysis: Analysis) {
  ...
  doc.text('Inner Vector', margin, 17);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(148, 163, 184);
  doc.text('Talent Assessment Report', margin, 25);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(148, 163, 184);
    doc.text('Generated by Inner Vector · Talent Assessment', margin, 292);
    doc.text(`${p} / ${totalPages}`, pageW - margin, 292, { align: 'right' });
  }

  const safeName = displayName.replace(/\s+/g, '_');
  doc.save(`TalentProfile_${safeName}.pdf`);
}
