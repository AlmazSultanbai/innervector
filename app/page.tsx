...
export default function LandingTrial() {
  ...
  return (
    <div className={`min-h-screen bg-white text-slate-900 ${display.className}`}>
      ...
      <header className="sticky top-0 z-30 bg-white/85 backdrop-blur-md border-b border-slate-200/70">
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/dashboard" className="hidden sm:inline text-sm font-semibold text-slate-600 hover:text-slate-900 transition-colors">Войти</Link>
      ...
      {/* Methodology — scientific credibility, placed prominently right under the hero */}
      <section className="border-b border-slate-100 bg-white">
        <div className="max-w-5xl mx-auto px-5 py-10">
          <p className="text-center text-xs font-bold uppercase tracking-widest mb-1" style={{ color: BLUE }}>Inner Vector Test</p>
          <p className="text-center text-sm font-bold uppercase tracking-widest text-slate-400 mb-5">Методология основана на</p>
          <div className="flex flex-wrap items-center justify-center gap-3">
            {[
              { n: 'Big Five', d: 'Большая пятёрка' },
              { n: 'HEXACO', d: 'Честность · Смирение' },
              { n: 'SDT', d: 'Теория самодетерминации' },
              { n: 'RIASEC', d: 'Типы Холланда' },
            ].map(m => (
              <div key={m.n} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full border border-slate-200 bg-white lift">
                <span className="text-sm font-extrabold" style={{ color: NAVY }}>{m.n}</span>
                <span className="text-sm text-slate-500 font-medium">{m.d}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="border-t border-slate-100">
        <div className="max-w-6xl mx-auto px-5 py-10 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 text-sm text-slate-500 font-medium flex-wrap justify-center">
            ...
            <Link href="/gallup" className="hover:text-slate-900 transition-colors">Отчёт Gallup</Link>
          </div>
