...
function VectorProfilePage() {
  ...
  const generateAnalysis = async (r: VectorResult) => {
    ...
    try {
      ...
      const res = await fetch('/api/vector-analyze', {
        ...
        body: JSON.stringify({ top5, top10, bottom5, lang: locale, full_name: r.full_name ?? '', result_id: r.id }),
      })
  ...
  // Auto-generate analysis on load if not yet generated
  useEffect(() => {
    if (result && !result.analysis && !autoGenerateFired[0]) {
      autoGenerateFired[1](true)
  ...
  return (
    <div className="min-h-screen bg-radial">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12 pb-24">
        ...
        {/* Coach Daniyar CTA */}
        <div className="mb-8 bg-white/2 border border-gold/20 rounded-2xl p-6 text-center print:hidden">
          <div className="text-[10px] tracking-widest text-gold/60 uppercase font-medium mb-2">
